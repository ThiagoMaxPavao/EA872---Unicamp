...
int processRequest(char *webspace, int socket_msg, int fd_log) {
    ...
    FD_ZERO(&fds); // Inicializa conjunto de fds limpo
    FD_SET(socket_msg, &fds); // Insere o fd do socket no conjunto
    timeout.tv_sec = 4; // 3 + 1 como tempo de timeout
    timeout.tv_usec = 0;

    ready = select(socket_msg + 1, &fds, (fd_set *)0, (fd_set *)0, &timeout);

    if(ready <= 0) {
        printf("Conexão aberta mas nada enviado em 4 segundos. Fechando...\n");
        return -1; // código de erro para indicar que conexão aberta não foi enviou conteúdos
    } 
